...
\begin{document}
    \maketitle 
    The scripts for both tasks were ran in NCC cluster using visdom for the visualization of the loss and accuracy plots. Scripts are included along with this report for the best architectures for each task.
    ...
    For the intial steps for this task intial codes from the lectures materials were used, the simple Autocoder was trained after modifing the datsets to be rescalled and normalized and similarly to VAE model. Figure \ref{pegasus} shows the outputs for generating a pegasus with those two models.
    \begin{figure}[H] 
        \centering
        \begin{minipage}{0.33\textwidth}
            \centering
            \includegraphics[width=1\textwidth]{{autoencoder_basic}.png}
            \subcaption{Basic Autocoder model}
            \label{cnn1acc}
        \end{minipage}
        \begin{minipage}{0.33\textwidth}
            \centering
            \includegraphics[width=1\textwidth]{{vae}.png}
            \subcaption{VAE model}
            \label{cnn1loss}
        \end{minipage}
        \caption{Generated Pegasus images}
        \label{pegasus}
    \end{figure}
    The VAE model showed much better results relatively to the basic autoencoder. The outline looks closer to a horse than the blob with the autoencoder image. For the next steps, the following architecture was used which is based on a paper by Larsen et al. \cite{vaegan}, Figure \ref{vaegan}. The idea behind this model is combine the GAN model features with the VAE model to provide a more realistic output rather than the blurry output from VAE model. 
    \begin{figure}[H] 
        \centering
        \begin{minipage}{0.49\textwidth}
            \centering
            \includegraphics[width=1\textwidth]{{vaegan}.png}
            \label{arch}
        \end{minipage}
        \caption{CNN with Residual Blocks architecture}
        \label{vaegan}
    \end{figure}
    For this case, the encoder and decoder are treated as seperate networks. The discriminator output is used for determining the loss for the decoder thus influencing the decoder weights. The weights for the encoder were carried as before with the VAE model. The weights for the discriminator were carried as with the GAN model. The output for the VAEGAN model shown in Figure \ref{vaegan:img}. The image is still not entirely clear but more details appeared.
    \begin{figure}[H] 
        \centering
        \begin{minipage}{0.30\textwidth}
            \centering
            \includegraphics[width=1\textwidth]{{vaegan_img}.png}
            \subcaption{VAEGAN model}
            \label{cnn1acc}
        \end{minipage}
        \begin{minipage}{0.30\textwidth}
            \centering
            \includegraphics[width=1\textwidth]{{res_vaegan}.png}
            \subcaption{VAEGAN with discriminator having 3 residual blocks model}
            \label{vaeganres}
        \end{minipage}
        \begin{minipage}{0.30\textwidth}
            \centering
            ...
            \label{cnn1loss}
        \end{minipage}
        \caption{comparison Generated Pegasus images}
        \label{vaegan:img}
    \end{figure}
    For the next step, as part of the discriminator network a residual block was included to measure if it will give a better output. The results shown in Figure \ref{vaeganres}. Still not much clear but again more details appear. And this appears to give the best results as the models complexity increases.  
